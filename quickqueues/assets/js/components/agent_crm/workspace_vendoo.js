var agent_crm = new Vue({
    ...
    methods: {
        ...
        vendoo_get_customer_data: function () {
            if (this.future_call.status == '' || !this.future_call.status) {
                this.future_call.status = 'closed';
            }
            if (this.call_in_progress.src != this.customer_info.phone) {
                console.log('New call, requesting customer info');
                this.customer_info = {};
                axios.post("https://cms.vendoo.ge/api/login?username=arcom&password=d381f78623e521250e2fa908692f533b", {})
                    .then(
                        response => {
                            this.customer_api_token = response.data.access_token;
                        })
                    .finally(
                        () => axios.post("https://cms.vendoo.ge/api/arcom/getCustomerData", {'phone': this.call_in_progress.src}, {
                                headers: {
                                    'Authorization': "bearer "+this.customer_api_token
                                }
                            })
                                .then(
                                    response => {
